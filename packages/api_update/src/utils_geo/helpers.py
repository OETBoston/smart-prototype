...
def consolidate_curb_segments(
    ...
    curb_segments['geography'] = curb_segments['geography'].apply(lambda x: wkb.loads(x, hex=True))
...
def get_linear_intersections(target_line, gdf: gpd.GeoDataFrame, geo_col='geometry'):
    ...
    # 2. Dimension Check:
    def is_linear_intersection(geom, target):
    ...
    mask = possible_matches[geo_col].apply(lambda x: is_linear_intersection(x, target_line))
