...
- **Arrow Events**: An `AWAY` direction activates the policy (adding it to the dictionary), while a `TOWARD` direction deactivates it. This is because encountering an arrow pointing `AWAY` from you means you are _entering_ its effective zone, whereas an arrow pointing `TOWARD` you means you are _exiting_ the zone.
...
- The outputs from the Forward and Backward passes are merged. If a policy is found to be active during _either_ pass, it applies to the segment.
