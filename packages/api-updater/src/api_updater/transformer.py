...
def extract_unique_policies(
    ...
    """Flattens the nested policy data,
    and returns unique policy dicts and a zone-to-json mapping."""
    df_exploded = (
        df_updates[["curb_zone_id", "policy_list"]].explode("policy_list").dropna()
    )
    df_exploded["policy_json"] = df_exploded["policy_list"].apply(
        lambda x: json.dumps(x, sort_keys=True)
    )

    # Map Zone ID -> List of Policy JSON strings
    zone_to_policies = (
        df_exploded.groupby("curb_zone_id")["policy_json"].apply(list).to_dict()
    )
...
def _create_policy_sub_elements(
    policy_id: uuid.UUID, policy_data: dict
...
def build_policy_tables(
    unique_policies: list[dict], run_time: pd.Timestamp
    ...
    for _i, policy in enumerate(unique_policies):
        p_id = uuid.uuid4()
    ...
    for row in df_updates.to_dict("records"):
        ...
        for p_json in policy_jsons:
            zone_policies.append(
                {"curb_zone_id": z_id, "curb_policy_id": json_to_id[p_json]}
            )
...
def transform_policy_updates(
    ...
    new_policies, new_rules, new_spans, new_rates, json_to_id_map = build_policy_tables(
        unique_policies, run_time
    )

    new_policies["policy_json"] = get_policy_json(
        new_policies, new_rules, new_spans, new_rates
    )
    new_policies["signature"] = get_policy_signatures(new_policies)

    # 3. Geo Processing
    new_zones, new_zone_policies = build_zone_tables(
        df_updates, zone_to_json_map, json_to_id_map, run_time
    )
    ...
    old_zones["geometry"] = old_zones["geometry"].apply(
    ...
    new_zone_policies["curb_policy_id"] = new_zone_policies["curb_policy_id"].replace(
        mapping_dict
    )
    new_policies = new_policies[
        ~new_policies["curb_policy_id"].isin(mapping_dict.keys())
    ]
    ...
    new_zone_policies_dict = (
        new_zone_policies.groupby("curb_zone_id")["curb_policy_id"].apply(set).to_dict()
    )
    old_zone_policies_dict = (
        old_zone_policies.groupby("curb_zone_id")["curb_policy_id"].apply(set).to_dict()
    )
    ...
    new_zone_policies["curb_zone_id"] = new_zone_policies["curb_zone_id"].replace(
        zone_id_replacements
    )
    ...
    new_zone_policies = new_zone_policies[
        new_zone_policies["curb_zone_id"].isin(active_zone_ids)
    ]
    old_zone_policies = old_zone_policies[
        old_zone_policies["curb_zone_id"].isin(active_zone_ids)
    ]

    active_policy_ids = set(new_zone_policies["curb_policy_id"]).union(
        set(old_zone_policies["curb_policy_id"])
    )

    # Helper to filter multiple dataframes at once
    def filter_by_policy(dfs, ids) -> list[pd.DataFrame]:
    ...
    curb_zone_policies = curb_zone_policies_diff[
        curb_zone_policies_diff["_merge"] == "left_only"
    ].drop(columns="_merge")
    ...
    return {
        ...
        "curb_policy_rules": {
            "table": curb_policy_rules,
            "keys": ["rule_id"],
        },
        "curb_policy_time_spans": {
            "table": curb_policy_time_spans,
            "keys": ["time_span_id"],
        },
        "curb_policy_rates": {
            "table": curb_policy_rates,
            "keys": ["rate_id"],
        },
    }
