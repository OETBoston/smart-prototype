...
This project can be installed and run using [uv](https://docs.astral.sh/uv/)
(a fast Python package and environment manager). 
...
```bash
git clone https://github.com/OETBoston/smart-curb-geometry-creation.git
cd smart-curb-geometry-creation/curb-generation
uv sync
```
...
## Entry Point (`main.py`)
`main.py` orchestrates the full pipeline in a deterministic, sequential order.
To execute this:
```python
uv run main.py
```
